...
const categoryColors = {
  frontend: { primary: '#00f5ff', secondary: '#00d4ff', accent: '#00b8e6' },
  backend: { primary: '#4ecdc4', secondary: '#3fb8b1', accent: '#36a8a1' },
  devops: { primary: '#6366f1', secondary: '#5855e6', accent: '#4f46e5' }
};
...
export default function Skills() {
  ...
  const initializeSkills = useCallback(() => {
    ...
    skillsRef.current = skillsData.map((skillData, index) => {
      ...
      return {
        name: skillData.name, category: skillData.category, x, y, originalX: x, originalY: y,
        vx: (Math.random() - 0.5) * 0.1, vy: (Math.random() - 0.5) * 0.1,
        radius: baseRadius, baseRadius, mass: baseRadius / 15,
  ...
  const updatePhysics = useCallback(() => {
    ...
    skills.forEach((skill, i) => {
      ...
      skill.vx += Math.sin(time + i * 0.3) * 0.15 * 0.001;
      skill.vy += Math.cos(time + i * 0.4) * 0.12 * 0.001;
      if (mouse.isActive) {
        const dx = mouse.x - skill.x;
        const dy = mouse.y - skill.y;
        const distance = Math.sqrt(dx * dx + dy * dy);
        if (distance < 120) {
          skill.energy = Math.max(skill.energy, ((120 - distance) / 120) * 1.5);
        } else { skill.energy *= 0.95; }
      } else { skill.energy *= 0.95; }
      ...
      for (let j = 0; j < skills.length; j++) {
        ...
        if (distance < minDistance && distance > 0) {
          ...
          skill.vx += Math.cos(angle) * force * 0.15;
          skill.vy += Math.sin(angle) * force * 0.15;
          if (overlap > 10) {
            skill.x += Math.cos(angle) * overlap * 0.3;
            skill.y += Math.sin(angle) * overlap * 0.3;
          }
        }
      }
      skill.vx *= 0.98;
      ...
      if (speed > 0.3) { skill.vx = (skill.vx / speed) * 0.3; skill.vy = (skill.vy / speed) * 0.3; }
      skill.x += skill.vx;
      skill.y += skill.vy;
      const margin = skill.radius * 2;
      if (skill.x < margin) skill.vx += (margin - skill.x) * 0.008;
      if (skill.x > width - margin) skill.vx -= (skill.x - (width - margin)) * 0.008;
      if (skill.y < margin) skill.vy += (margin - skill.y) * 0.008;
      if (skill.y > height - margin) skill.vy -= (skill.y - (height - margin)) * 0.008;
      if (speed > 0.05) skill.trail.unshift({ x: skill.x, y: skill.y, alpha: 1 });
      if (skill.trail.length > 8) skill.trail.pop();
      skill.trail.forEach((point, index) => { point.alpha = 1 - (index / skill.trail.length); });
  ...
  const render = useCallback(() => {
    ...
    skillsRef.current.forEach((skill) => {
      ...
      ctx.shadowBlur = 8 + skill.energy * 15;
      ctx.beginPath();
      ctx.arc(skill.x, skill.y, skill.radius, 0, Math.PI * 2);
      const gradient = ctx.createRadialGradient(skill.x - skill.radius * 0.3, skill.y - skill.radius * 0.3, 0, skill.x, skill.y, skill.radius);
      ...
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(skill.x - skill.radius * 0.3, skill.y - skill.radius * 0.3, skill.radius * 0.3, 0, Math.PI * 2);
  ...
  const handleMouseLeave = useCallback(() => { mouseRef.current.isActive = false; }, []);
  ...
  const handleDoubleClick = useCallback(() => { initializeSkills(); }, [initializeSkills]);

  return (
    <section id="skills" ref={sectionRef as React.RefObject<HTMLElement>} className="reveal">
      <div className="section-label">Skills</div>
      <h2 className="section-title">Technical <span className="accent">Expertise</span></h2>
      <p className="section-subtitle">Hover over the bubbles to interact with my skill set</p>
      <div ref={containerRef} className="skills-canvas-container" style={{ height: dimensions.height }}>
        <canvas
          ref={canvasRef}
